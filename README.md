...
线上 demo 实例（免卡免费）：`streamlit_app.py` 提供审批台 UI（mock LLM + 只读沙箱），部署到 Streamlit Community Cloud（GitHub 仓库直连，免费）。线上地址：<https://jasonw43-coding-agent-harness-streamlit-app-gpxdmb.streamlit.app/>。项目另提供 `Dockerfile` 支持容器部署（Render 等，端口 7860）。
